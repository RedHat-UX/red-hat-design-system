...
import style from './rh-accordion.css';

function isRhAccordionPanel(el?: EventTarget|null): el is RhAccordionPanel {
 ...
 *
 * @fires {CustomEvent<{ toggle: RhAccordionHeader, panel: RhAccordionPanel }>} rh-accordion:expand - when a panel expands {@deprecated Use `expand`}
 * @fires {CustomEvent<{ toggle: RhAccordionHeader, panel: RhAccordionPanel }>} rh-accordion:collapse - when a panel collapses {@deprecated Use `collapse`}
 *
 * @slot
 *       Place the `rh-accordion-header` and `rh-accordion-panel` elements here.
 *
 * @cssproperty {[ <length> | <percentage> ]{1,4}}--rh-accordion--Padding
...
export class RhAccordion extends LitElement {
   ...
   *
   * ```html
   * <rh-accordion history id="lorem-ipsum">
   *   <rh-accordion-header>
   *     <h3>Accordion 1</h3>
   *   </rh-accordion-header>
   *   <rh-accordion-panel>
   ...
   *
   * ```html
   * <rh-accordion expanded-index="2,3">
   *   ...
   * </rh-accordion>
   * ```
   */
  ...
  private _expandHeader(header: RhAccordionHeader) {
  ...
  private async _expandPanel(panel: RhAccordionPanel) {
    if (!panel) {
  ...
  private _collapseHeader(header: RhAccordionHeader) {
    const index = this._getIndex(header);
  ...
  private async _collapsePanel(panel: RhAccordionPanel) {
    if (!panel) {
  ...
  private async _animate(panel: RhAccordionPanel, start: number, end: number) {
    if (panel) {
  ...
  private async _keydownHandler(evt: KeyboardEvent) {
    ...
    let newHeader: RhAccordionHeader;
  ...
  private _allHeaders(): RhAccordionHeader[] {
    return Array.from(this.children).filter(RhAccordion.isHeader);
  }

  private _allPanels(): RhAccordionPanel[] {
    return Array.from(this.children).filter(RhAccordion.isPanel);
  }

  private _panelForHeader(header: RhAccordionHeader) {
  ...
  public expand(index: number) {
    ...
    this.dispatchEvent(deprecatedCustomEvent('rh-accordion:expand', { toggle, panel }));
  }
  ...
  public collapse(index: number) {
    ...
    this.dispatchEvent(deprecatedCustomEvent('rh-accordion:collapse', { toggle, panel }));
  }
