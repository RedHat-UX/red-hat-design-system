...
test.describe(tagName, () => {
  test('snapshot', async ({ page }) => {
    ...
    async function elementSuite(selector: string, variant: string) {
      ...
      test
          ...
          .toMatchSnapshot({ name: `${variant} 0.png` });
      ...
      const { x, width, y, height } =
        (await element.boundingBox()) || defaultBounds;
    ...
    for (const selector of Array.from(
      { length: 5 },
      (_, i) => `#cta-variants > rh-cta:nth-of-type(${i + 1})`
    )) {
    ...
    for (const selector of Array.from(
      { length: 2 },
      (_, i) => `#cta-variants #brick rh-cta:nth-of-type(${i + 1})`
    )) {
      const [, nth] = selector.match(/nth-of-type\((\d)\)/)!;
      await elementSuite(selector, ['brick', 'brick icon'][parseInt(nth) - 1]);
    }
  });
  ...
  for (const { selector, icon: iconName } of variantsWithIcons) {
    test(`cta with selector ${selector} should have a visible ${iconName} icon`, async ({
      page,
    }) => {
      const componentPage = new PfeDemoPage(page, tagName);
      await componentPage.navigate('/elements/call-to-action/demo/variants/');
  ...
  for (const selector of variantsWithoutIcons) {
    test(`cta with selector ${selector} should not have an icon`, async ({
      page,
    }) => {
      const componentPage = new PfeDemoPage(page, tagName);
      await componentPage.navigate('/elements/call-to-action/demo/variants/');
