...
} from '@rhds/elements/rh-site-status/rh-site-status.js';
...
describe('<rh-site-status>', function() {
  ...
  describe('simply instantiating', function() {
    beforeEach(async function() {
      ...
      element = await createFixture<RhSiteStatus>(
        html`<rh-site-status></rh-site-status>`
      );
    ...
    it('should upgrade', async function() {
      ...
      expect(element)
          ...
          .and.to.be.an.instanceOf(RhSiteStatus);
    });
  });
  ...
  describe('when endpoint returns a 404', function() {
    beforeEach(async function() {
      fetchStub = stub(window, 'fetch');
      fetchStub.resolves(
        new Response('', { status: 404, statusText: '404 Not Found' })
      );
      element = await createFixture<RhSiteStatus>(
        html`<rh-site-status></rh-site-status>`
      );
