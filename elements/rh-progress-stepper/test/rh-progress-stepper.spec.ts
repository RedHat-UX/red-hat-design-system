...
import {
  RhProgressStepper,
  RhProgressStep,
} from '@rhds/elements/rh-progress-stepper/rh-progress-stepper.js';
...
describe('<rh-progress-stepper>', function() {
  describe('simply instantiating', function() {
    ...
    it('imperatively instantiates', function() {
      expect(document.createElement('rh-progress-stepper')).to.be.an.instanceof(
        RhProgressStepper
      );
      expect(document.createElement('rh-progress-step')).to.be.an.instanceof(
        RhProgressStep
      );
    });

    it('should upgrade', async function() {
      element = await createFixture<RhProgressStepper>(
        html`<rh-progress-stepper></rh-progress-stepper>`
      );
      const klass = customElements.get('rh-progress-stepper');
      expect(element)
          .to.be.an.instanceOf(klass)
          .and.to.be.an.instanceOf(RhProgressStepper);
    });
  });
  ...
  describe('with more than one step marked as active', function() {
    ...
    it('sets the last active step as aria-current', async function() {
      ...
      const [hasAriaCurrent] = Array.from(
        element.querySelectorAll('rh-progress-step'),
        x => x.shadowRoot?.querySelector('[aria-current]')
      ).filter(x => !!x);
      // super brittle: much better if we could assert on the AT tree
      const [assignedNode] =
        hasAriaCurrent?.querySelector('slot')?.assignedNodes() ?? [];
