...
export type NavigationPrimaryPalette = Extract<
  ColorPalette,
  'lightest' | 'darkest'
>;
...
export class RhNavigationPrimary extends LitElement {
      ...
      <div id="container" class="${classMap(classes)}">
        <div id="bar">
            ...
            <slot name="logo">
              <a href="/">
                <svg preserveAspectRatio="xMinYMid slice" viewBox="0 0 613 145">
                  ...
                  ></path>
                  <path
                    fill="light-dark(var(--rh-color-text-primary-on-light, #151515), var(--rh-color-text-primary-on-dark, #ffffff))"
          ...
          <details
            ...
            @focusout="${this.#onHamburgerFocusOut}"
          >
  ...
  #primaryItems(): RhNavigationPrimaryItem[] {
    return Array.from(
      this.querySelectorAll('rh-navigation-primary-item:not([slot])')
    );
  }
  ...
  #closeSecondaryDropdowns() {
    this.#openSecondaryDropdowns.forEach(
      (dropdown: RhNavigationPrimaryItem) => {
        dropdown.hide();
      }
    );
  }
