...
export class PrimaryNavOverlayChangeEvent extends ComposedEvent {
  constructor(
    public open: boolean,
 ...
 * @fires {primaryNavOverlayChangeEvent} overlay-change -
 *                                         Fires when an dropdown is opened or closed in desktop
 *                                         view or when the mobile menu button is toggled in mobile
 *                                         view.
 * @cssprop {<integer>} --rh-navigation-primary-z-index - z-index of the navigation-primary {@default `102`}
 * @cssprop {<integer>} --rh-navigation-primary-overlay-z-index - z-index of the navigation-primary-overlay {@default `-1`}
 */
...
export class RhNavigationPrimary extends LitElement {
  ...
  #tabindex = new RovingTabindexController(this, {
    getItems: () => this._nav?.flatMap(slotted =>
      Array.from(slotted.querySelectorAll<HTMLAnchorElement>(`:is(rh-navigation-primary-dropdown,
                                                                  rh-primary-nav-dropdown) > a,
                                                              [slot="nav"] > li > a`))) ?? [],
  });
   ...
   * If isMobile is set dispatch an primaryNavOverlayChangeEvent event
   * to open the overlay
   * @param event when a dropdown tries to expand
   */
  #onExpandRequest(event: Event) {
    if (event instanceof PrimaryNavDropdownExpandEvent) {
  ...
  #allDropdowns(): RhNavigationPrimaryDropdown[] {
    return Array.from(
      this.querySelectorAll('rh-navigation-primary-dropdown, rh-primary-nav-dropdown')
    );
  }
   ...
   * @param event primary nav overlay change event
   */
  #onOverlayChange(event: Event) {
    if (event instanceof PrimaryNavOverlayChangeEvent) {
  ...
  #upgradeAccessibility(): void {
    ...
    if (this.hasAttribute('aria-label')) {
      this.#label = this.getAttribute('aria-label') ?? 'primary';
  ...
  #toggleMobileMenu() {
    ...
    this.dispatchEvent(new PrimaryNavOverlayChangeEvent(this.mobileMenuExpanded, this));
  }
