...
const [
  ...
  HamburgerItem4Name,
  HamburgerItem5Name,
] = ['Ham 1', 'Ham 2', 'Ham 3', 'Ham 4', 'Ham 5'];
const NAV = html`
  <rh-navigation-primary>
    <rh-navigation-primary-item
      variant="dropdown"
      summary="${HamburgerItem1Name}"
    >
...
describe('<rh-navigation-primary>', function() {
  describe('simply instantiating', function() {
    it('imperatively instantiates', function() {
      expect(
        document.createElement('rh-navigation-primary')
    ...
    it('should upgrade', async function() {
      ...
      expect(element)
          ...
          .and.to.be.an.instanceOf(RhNavigationPrimary);
    });
  });
  ...
  describe('resizing viewports', function() {
    ...
    describe('compact viewport', function() {
      ...
      it('menu should be closed', async function() {
        ...
        expect(snapshot)
            .axQuery({ name: 'Menu' })
            .to.not.have.property('expanded', true);
      });
          ...
          describe('toggle open', function() {
            ...
            describe('toggling a standalone dropdown', function() {
              beforeEach(async function() {
                 ...
                 */
                const item6 = element.querySelector(
                  'rh-navigation-primary-item[summary="Item 6"]'
                );
              ...
              it('should close the hamburger menu', async function() {
                const snapshot = await a11ySnapshot();
                expect(snapshot).to.not.have.axQuery({
                  name: 'Menu',
                  expanded: false,
                });
                expect(snapshot).to.not.have.axQuery({
                  name: HamburgerItem1Name,
                });
                expect(snapshot).to.not.have.axQuery({
                  name: HamburgerItem2Name,
                });
                expect(snapshot).to.not.have.axQuery({
                  name: HamburgerItem3Name,
                });
                expect(snapshot).to.not.have.axQuery({
                  name: HamburgerItem4Name,
                });
                expect(snapshot).to.not.have.axQuery({
                  name: HamburgerItem5Name,
                });
              });
              ...
              it('should still have an open overlay', async function() {
                ...
                const overlay = element.shadowRoot?.querySelector(
                  'rh-navigation-primary-overlay[open]'
                );
  ...
  describe('dark mode', function() {
    ...
    it('should have a dark themed menu bar', function() {
      const bar = element.shadowRoot?.querySelector('#bar') as
        | HTMLElement
        | undefined;
      expect(
        getComputedStyle(bar!).getPropertyValue('background-color')
      ).to.be.equal('rgb(21, 21, 21)'); /* rgb(21, 21, 21) equiv #151515 */
    });
  });
});
