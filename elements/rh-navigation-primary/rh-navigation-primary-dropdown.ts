...
import { RhNavigationPrimaryMenu } from './rh-navigation-primary-menu.js';

export class PrimaryNavDropdownExpandEvent extends ComposedEvent {
  constructor(
 ...
 * @fires { PrimaryNavDropdownExpandEvent } change - Fires when a dropdown is clicked
 */
...
export class RhNavigationPrimaryDropdown extends LitElement {
   ...
   * and then dispatch that value in a PrimaryNavDropdownExpandEvent
  ...
  private _clickHandler(event: MouseEvent) {
    ...
    this.dispatchEvent(new PrimaryNavDropdownExpandEvent(this.expanded, this));
  }
  ...
  #open(): void {
    ...
    // menu as a RhNavigationPrimaryMenu in the slotted child is specific to rh-navigation-primary.
    // If this component is abstracted to a standalone component. The RhNavigationPrimaryMenu
    // could possibly become a sub component of the abstraction instead.
    const menu = this.#slots.getSlotted('menu').find(child =>
      child instanceof RhNavigationPrimaryMenu
    ) as RhNavigationPrimaryMenu;
    menu.visible = true;
  }
  ...
  #close() {
    ...
    // The RhNavigationPrimaryMenu could possibly become a sub component of the abstraction instead.
    const menu = this.#slots.getSlotted('menu').find(
      (child: Node): child is RhNavigationPrimaryMenu =>
        child instanceof RhNavigationPrimaryMenu);
    if (menu) {
      menu.visible = false;
    }
