...
describe('<rh-alert>', function() {
    ...
    element = await fixture<RhAlert>(html`
      <rh-alert state="default">
        ...
        <p>
          ...
          eleifend elit sed est egestas, a sollicitudin mauris tincidunt.
        </p>
  ...
  it('should upgrade', async function() {
    ...
    expect(element).to.be.an.instanceOf(klass).and.to.be.an.instanceOf(RhAlert);
  });
  ...
  describe('dismissable <rh-alert>', async () => {
    ...
    beforeEach(async function() {
      elementCloseButton =
        dismissableElement.shadowRoot?.querySelector('#close-button');
    });
  ...
  for (const [state, bgtoken] of Object.entries({
    ...
    describe(`state="${state}"`, function() {
      ...
      beforeEach(async function() {
        element = await createFixture(
          html`<rh-alert state="${state}">Content</rh-alert>`
        );
      });
      it('uses the correct background color', function() {
        ...
        const actual = getComputedStyle(
          element.shadowRoot!.getElementById('container')!
        ).backgroundColor;
    ...
    describe(`state="${state.toUpperCase()}" case insensitive`, function() {
      ...
      beforeEach(async function() {
        element = await createFixture(
          html`<rh-alert state="${state.toUpperCase()}">Content</rh-alert>`
        );
      });
      it('uses the correct background color', function() {
        // it would be better to assert the color value at a given pixel offset from the element boundary
