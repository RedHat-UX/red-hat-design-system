...
export class RhCard extends LitElement {
  ...
  override render() {
    ...
    if (isStandardPromo) {
      computedPalette = `${`${this.colorPalette ?? 'lightest'}`.replace(
        PALETTE_RE,
        ''
      )}er` as 'lighter' | 'darker';
    } else if (isPromo) {
      computedPalette = `${`${this.colorPalette ?? 'lightest'}`.replace(
        PALETTE_RE,
        ''
      )}est` as 'lightest' | 'darkest';
    } else {
    ...
    const header = html` <!-- The header for the card. Contains the header slot. -->
      <div id="header" part="header" class="${classMap({ empty: !hasHeader })}">
        <!--
    ...
    const footer = html` <!-- The footer for the card. Contains the footer slot. -->
      <div id="footer" part="footer" class="${classMap({ empty: !hasFooter })}">
