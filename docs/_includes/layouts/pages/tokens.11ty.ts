...
export default class TokensPage extends Renderer<Data> {
  data() {
    return {
      ...
      permalink: (data: Data): string | null =>
        !data.tokenCategory?.slug ?
          null
      ...
      eleventyComputed: {
        title: (data: Data) =>
          data.title
          || `${capitalize(
            data.tokenCategory.title || data.tokenCategory.slug
          )} tokens`,
      },
      pagination: {
        size: 1,
  ...
  #getTokenLightness(token: DesignToken, palette: 'light' | 'dark') {
    ...
    const value =
      meta?.$value || !Array.isArray(meta?.original?.$value) ?
        ''
        : meta?.original.$value.find(
          (x: string | number) =>
            x.toString().endsWith(`${palette}est}`)
              || x.toString().endsWith(`${palette}}`)
        );
    const derefed = `--rh-${value
        ?.toString()
        .replace(/{(.*)}/, '$1')
        .replace(/\./g, '-')}` as const;
    const derefedToken = meta?.$value ?
      meta
      : tokensMeta.get(derefed as TokenName);
  ...
  async #renderTable(options: Options) {
    ...
    const tokens = Object.values(options.tokens).filter(
      (x: unknown): x is DesignToken =>
    ...
    return html` <rh-table>
            ...
            ${(
              await Promise.all(
                tokens
                    ...
                    .map(token => this.#renderTokenRow(token, options))
              )
            ).join('\n')}
          </tbody>
        </table> </rh-table
      >${!deprecatedTokens.length ?
        ''
        : html` <rh-disclosure
            class="deprecated-tokens"
            summary="Deprecated tokens"
          >
            <rh-table>
              <table>
                <thead>
                ...
                <tbody>
                  ${(
          await Promise.all(
            deprecatedTokens.map(token =>
              this.#renderTokenRow(token, options)
            )
          )
        ).join('\n')}
                </tbody>
  ...
  async #renderTokenRow(token: DesignToken, { name, tokens }: Options) {
    ...
    return html` <tr
      ...
      style="${styleMap({
        ...
        '--samp-font-family': isFamily ?
          token.$value
          : 'var(--rh-font-family-body-text)',
        '--samp-font-size': isSize ?
          token.$value
          : 'var(--rh-font-size-heading-md)',
        '--samp-font-weight': isWeight ?
          token.$value
          : 'var(--rh-font-weight-body-text-regular)',
        [`--samp-${token.$type === 'dimension' ? `${name}-size` : name}`]:
          token.$value,
        [`${
          token.$type === 'dimension' && token.attributes?.category === 'space' ?
            `--samp-${name}-color`
            : ``
        }`]: isSpace ?
          token.original?.['$extensions']['com.redhat.ux'].color
          : '',
      })}"
    >
      <td>
        <samp class="${classes}">
          ${!isSpace ?
            ''
            : html`<span class="${parseInt(token.$value) < 16 ? `offset` : ''}"
                >${parseInt(token.$value)}</span
              >`}
          ${isColor && token.path.includes('text') ?
            'Aa'
            : isFont ?
            example || (token.attributes?.aliases as string[])?.[0] || 'Aa'
            : name === 'breakpoint' ?
            html` <img
                src="/assets/breakpoints/device-${token.name}.svg"
                role="presentation"
              />`
            : example}
        </samp>
      ...
      <td>
        ${isDimension ?
          html`<code>${token.$value}<code></code></code>`
          : isColor ?
          html`<code style="--color: ${token.$value}">${token.$value}</code> `
          : isWeight ?
          html` <code class="numerical">${token.$value}</code>
              <code class="common"
                >${(token.attributes?.aliases as string[])?.[0] ?? ''}</code
              >`
  ...
  #makeSwatches(palette: 'light' | 'dark') {
    return (token: DesignToken) => {
      const suffix =
        token.name === 'rh-color-surface' ? `${palette}est` : `on-${palette}`;
      const style = `--swatch-color: var(--${token.name}-${suffix})`;
      const classes = `swatch on-${palette} ${classMap(
        this.#getTokenLightness(token, palette)
      )}`;
      return token.path.includes('text') ?
        html` <samp class="${classes} font" style="${style}">
            <span>Aa</span> <span>--${token.name}</span>
          </samp>`
        : token.path.includes('icon') ?
        html` <samp class="${classes} icon" style="${style}">
            <rh-icon icon="unknown-fill" set="ui"></rh-icon>
            <span>--${token.name}</span>
          </samp>`
        : token.path.includes('border') ?
        html` <samp class="${classes} border" style="${style}">
            <span>--${token.name}</span>
          </samp>`
        : html` <samp class="${classes} color" style="${style}">
            <span>--${token.name}</span>
          </samp>`;
    };
  }
  ...
  async #renderContent(options: Options) {
    ...
    return html`
      ...
      ${await this.#renderTable(options)} ${await this.#renderChildren(options)}
  ...
  async #renderTokenDeprecationReason({ $deprecated }: DesignToken) {
    return !$deprecated ?
      ''
      : $deprecated === true ?
      html`<div class="deprecated"><strong>Deprecated</strong></div>`
      : html`<div class="deprecated">
          ${await this.renderTemplate($deprecated, 'md')}
        </div>`;
  }
  ...
  async #renderCategory(options: Options) {
    ...
    if (options.level >= 4) {
      ...
      return html`
        <div
          id="${options.path.replace('.', '-')}-section"
      ...
      <script type="module" data-helmet>
        ...
        async function updateSwatches() {
          ...
          for (const swatch of swatches) {
            const value =
              getComputedStyle(swatch).getPropertyValue("--swatch-color");
