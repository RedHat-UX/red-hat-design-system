...
export default class Feedback extends Renderer<Data> {
  async render({ doc, title, pfeconfig }: Data & GlobalData) {
    ...
    const related = [...new Set(relatedItems)]
        .map(x => {
    ...
    return dedent(html`
      ...
      <uxdot-feedback
        >${!related.length ?
          html` <h2>Other libraries</h2>
              <p>
                To learn more about our other libraries, visit the
                <a href="/get-started/">getting started page</a>.
              </p>`
          : html` <h2>Related elements or patterns</h2>
