...
function getCategories(token) {
  const categories =
    tokens.get(token)?.path?.filter(cat => categorySlugs.has(cat)) ?? [];
...
if (searchResults.length) {
  const resultsHTML = (numResults, term) =>
    html` <p>Showing ${numResults} results for "${term}"</p> `;
  ...
  const noResultsHTML = term =>
    html` <p>No search results for "${term}"</p> `;
