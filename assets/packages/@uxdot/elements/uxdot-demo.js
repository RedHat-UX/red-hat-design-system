...
import { css } from "lit";
const styles = css `:host{display:block}iframe{border:var(--rh-border-width-sm) solid var(--rh-color-border-subtle);block-size:400px;max-block-size:var(--height);inline-size:100%;box-sizing:border-box}#container{--height:825px;display:flex;flex-direction:column;gap:var(--rh-space-2xl)}#container:not(:defined){opacity:0}#code rh-tabs>rh-tab-panel{--_panel-padding:0}#code::part(container){box-sizing:border-box}#code::part(body),#code::part(header){margin:0;border-block-end:var(--rh-border-width-sm) solid var(--rh-color-border-subtle)}#code::part(footer){margin:0;flex-flow:row wrap;gap:var(--rh-space-md);padding:var(--rh-space-md)}@container (min-width: 992px){#code::part(footer){gap:var(--rh-space-lg);padding:var(--rh-space-lg)}}#code rh-button[slot=footer]{display:inline-block}:is(#code rh-button[slot=footer])::part(button){text-align:start}#code a[slot=footer]{display:inline-flex;align-items:center;white-space:nowrap;gap:var(--rh-space-sm);padding-inline:calc(var(--rh-space-lg)*.75) var(--rh-space-lg);padding-block:var(--rh-space-sm);text-decoration:none}:is(#code a[slot=footer]):focus-visible{outline:var(--rh-border-width-md,2px) solid var(--rh-color-interactive-primary-default);border-radius:var(--rh-border-radius-default,3px)}#code ::slotted(rh-code-block){max-inline-size:100%!important}#preview{display:flex;flex-direction:column}#knobs-drawer{display:none}:host([attribute-knobs]) iframe{border-end-start-radius:0;border-end-end-radius:0}:host([attribute-knobs]) #knobs-drawer{position:relative;display:block;margin-block-start:-1px;border-end-start-radius:var(--rh-border-radius-default);border-end-end-radius:var(--rh-border-radius-default)}:host([attribute-knobs]) #knobs{display:grid;grid-template-columns:1fr;gap:var(--rh-space-2xl)}@media (min-width:768px){:host([attribute-knobs]) #knobs{grid-template-columns:1fr 1fr}}@media (min-width:992px){:host([attribute-knobs]) #knobs{grid-template-columns:repeat(3,1fr)}}@media (min-width:1200px){:host([attribute-knobs]) #knobs{grid-template-columns:repeat(4,1fr)}}:host([attribute-knobs]) #code{block-size:auto}:is(:host([attribute-knobs]) #code)::part(body){block-size:100%}:is(:host([attribute-knobs]) #code) .code-tabs{border:0;border-radius:var(--rh-border-radius-default)}:is(:is(:host([attribute-knobs]) #code) .code-tabs)::part(tabs-container){background:light-dark(var(--rh-color-surface-lightest),var(--rh-color-surface-darker))}:is(:is(:host([attribute-knobs]) #code) .code-tabs) rh-tab-panel{padding:0;border-radius:0}:is(:host([attribute-knobs]) #code) ::slotted(rh-code-block){--rh-border-radius-default:0;--rh-border-width-sm:0px;border-width:0}:host([attribute-knobs]) :host(:fullscreen) #container{block-size:100vh;justify-content:space-between}:is(:host([attribute-knobs]) :host(:fullscreen) #container) iframe{flex:1 1 auto;max-block-size:unset}:is(:host([attribute-knobs]) :host(:fullscreen) #container) #code{block-size:max-content}`;
