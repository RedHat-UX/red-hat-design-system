...
import { css } from "lit";
const styles = css `button{color:inherit;border-radius:var(--rh-border-radius-default);border-width:0;background:none;display:inline-flex;align-items:center;gap:var(--rh-space-xs);padding-inline:var(--rh-space-xs)}code{padding:var(--rh-space-xs) var(--rh-space-md);background:var(--rh-color-surface-light);font-size:var(--rh-font-size-code-md);font-weight:var(--rh-font-weight-code-regular);font-family:var(--rh-font-family-code);line-height:var(--rh-line-height-code)}.visually-hidden{border:0;clip:rect(0,0,0,0);block-size:1px;margin:-1px;overflow:hidden;padding:0;position:absolute;white-space:nowrap;inline-size:1px}:host(.icon-only) code,:host(:empty) code{display:none}:is(rh-icon,#caption){display:none}:host(:state(--rendered)) button:is(:focus,:active,:hover),:host(:state(--rendered)) button:is(:focus,:active,:hover) code{background:var(--rh-color-interactive-primary-hover-on-dark);opacity:1}:host(:state(--rendered)) :is(rh-icon,#caption){display:initial}`;
...
_UxdotCopyButton_internals = new WeakMap(), _UxdotCopyButton_instances = new WeakSet(), _UxdotCopyButton_onClick = async function _UxdotCopyButton_onClick() {
    const text = this.copy ?? this.textContent ?? '';
    const message = text.trim();
    await navigator.clipboard.writeText(message);
    RhAlert.toast({ heading: 'Copied', message });
};
UxdotCopyButton.properties = {
    copy: {},
    icon: {}
};
UxdotCopyButton.styles = [styles];
customElements.define("uxdot-copy-button", UxdotCopyButton);
