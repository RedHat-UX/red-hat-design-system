...
_UxdotPattern_instances = new WeakSet(), _UxdotPattern_onChange = function _UxdotPattern_onChange(event) {
    if (event instanceof ContextChangeEvent) {
        this.colorPalette = event.colorPalette;
    }
};
UxdotPattern.properties = {
    colorPalette: { reflect: true, attribute: 'color-palette' },
    target: { reflect: true },
    src: { reflect: true },
    cssSrc: { reflect: true, attribute: 'css-src' },
    jsSrc: { reflect: true, attribute: 'js-src' },
    noColorPicker: { type: Boolean, attribute: 'no-color-picker' },
