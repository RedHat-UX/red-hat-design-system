 ...
 */
let RhCard = RhCard_1 = _a = class RhCard extends LitElement {
    constructor() {
        super(...arguments);
        /**
        ...
        _RhCard_slots.set(this, new SlotController(this, ...RhCard_1.slots));
    }
    render() {
        const { on = '', colorPalette = '', variant = '' } = this;
        const slots = Object.fromEntries(RhCard_1.slots.map(slot => [slot ?? 'body', __classPrivateFieldGet(this, _RhCard_slots, "f").hasSlotted(slot)]));
        const promo = this.variant === 'promo';
        const standard = promo && slots.body && !slots.image && !slots.header;
        const computedPalette = !standard ? colorPalette
...
RhCard.slots = ['header', 'image', null, 'footer'];
__decorate([
