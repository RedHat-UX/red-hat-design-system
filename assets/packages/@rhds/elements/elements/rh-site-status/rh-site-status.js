var _RhSiteStatus_instances, _a, _RhSiteStatus_logger, _RhSiteStatus_text, _RhSiteStatus_icon, _RhSiteStatus_isLoading, _RhSiteStatus_getStatus;
var RhSiteStatus_1;
...
let RhSiteStatus = RhSiteStatus_1 = _a = class RhSiteStatus extends LitElement {
    constructor() {
        ...
        _RhSiteStatus_text.set(this, '');
        _RhSiteStatus_icon.set(this, html ``);
        _RhSiteStatus_isLoading.set(this, true);
    }
    async connectedCallback() {
        super.connectedCallback();
        await __classPrivateFieldGet(this, _RhSiteStatus_instances, "m", _RhSiteStatus_getStatus).call(this);
    }
    render() {
        const { on = '' } = this;
        return html `
      <div id="container" class="${classMap({ [on]: !!on })}">
        <a href="https://status.redhat.com/" aria-busy="${__classPrivateFieldGet(this, _RhSiteStatus_isLoading, "f") ? 'true' : 'false'}" aria-live="polite">
          ${__classPrivateFieldGet(this, _RhSiteStatus_isLoading, "f") ? html `<rh-spinner size="sm"></rh-spinner><span><slot name="loading-text">Loading</slot></span>` : html `
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="17" viewbox="0 0 16 17" fill="none">
              ${__classPrivateFieldGet(this, _RhSiteStatus_icon, "f")}
            </svg>
            <span>${__classPrivateFieldGet(this, _RhSiteStatus_text, "f")}</span>
          `}
        </a>
      </div>
    `;
...
_RhSiteStatus_text = new WeakMap();
_RhSiteStatus_icon = new WeakMap();
_RhSiteStatus_isLoading = new WeakMap();
_RhSiteStatus_instances = new WeakSet();
_RhSiteStatus_getStatus = async function _RhSiteStatus_getStatus() {
    try {
        const data = await fetch('https://status.redhat.com/index.json', {
            mode: 'cors',
            cache: 'no-cache',
            headers: {
        ...
        if (!RhSiteStatus_1.isApiStatus(data)) {
            throw new Error('Invalid status data');
        }
        const statusText = data.status.description;
        __classPrivateFieldSet(this, _RhSiteStatus_text, textMap[statusText] || statusText, "f");
        __classPrivateFieldSet(this, _RhSiteStatus_icon, statusIconsMap[statusMap[data.status.indicator]], "f");
        __classPrivateFieldSet(this, _RhSiteStatus_isLoading, false, "f");
        this.requestUpdate();
    }
    catch (error) {
        __classPrivateFieldGet(this, _RhSiteStatus_logger, "f").warn('Error loading site status:', error);
        __classPrivateFieldSet(this, _RhSiteStatus_text, 'Error loading status', "f");
        __classPrivateFieldSet(this, _RhSiteStatus_icon, statusIconsMap['danger'], "f");
        __classPrivateFieldSet(this, _RhSiteStatus_isLoading, false, "f");
        this.requestUpdate();
    }
};
RhSiteStatus.styles = [styles];
RhSiteStatus.isApiStatus = (data) => {
